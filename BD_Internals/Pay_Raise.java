...
public class Pay_Raise {
	...
	public void map(LongWritable key, Text value, OutputCollector<Text, IntWritable> output, Reporter reporter) throws IOException {
		...
		if (tokens[3].matches ("TRUE")) { 
			output.collect (new Text (tokens[3]), one);
		}
	}
	}
	...
	public void reduce(Text key, Iterator<IntWritable> values, OutputCollector<Text, IntWritable> output, Reporter reporter) throws IOException { //{little: {1,1}} 
		...
		output.collect(new Text("Eligible for Pay Raise: "), new IntWritable(sum));
		}
	}	
	//DRIVER CODE
	public static void main(String[] args) throws Exception {
		JobConf conf = new JobConf(Pay_Raise.class);
		conf.setJobName("payraise");
